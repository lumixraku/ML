...
"""
ad_bi_antispam_form_service_log_daily 是模型判断结果的数据
"""
if __name__ == '__main__':


    # sql = "SELECT req_id from ad_bi_antispam_form_service_log_daily where date = '20171104' and judge = 1002 and req_id != '' and req_id is not null limit 1000"
    ...
    for rec in rs:
        ...
        behaviors = dataobj.get('behaviors')
        feature = _uac_feature(behaviors)

        features.append(feature)
        def _matrix_feature(feature):
            rs = range(9)

            rs[0] = feature.get('phone_yes_or_no', 0)
            rs[1] = feature.get('first_show_time', 0)
            rs[2] = feature.get('total_submit_time', 0)
            rs[3] = feature.get('name_time', 0)
            rs[4] = feature.get('first_focus_time', 0)
            rs[5] = feature.get('captcha', 0)
            rs[6] = feature.get('page_stay_time', 0)
            rs[7] = feature.get('telphone_time', 0)
            rs[7] = feature.get('component_count', 0)
            rs[8] = feature.get('screen_pos', 0)
            return rs

        print judge, _matrix_feature(feature)



# /home/luonan/repos/ML/spamdata
